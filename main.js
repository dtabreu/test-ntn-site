...
if (form) {
  form.addEventListener('submit', e => {
    ...
    if (!nome || !cidade) {
      ...
      let errMsg = firstEmpty.nextElementSibling;
      if (!errMsg || !errMsg.classList.contains('field-error')) {
        errMsg = document.createElement('span');
        errMsg.className = 'field-error';
        errMsg.setAttribute('role', 'alert');
        firstEmpty.after(errMsg);
      }
      errMsg.textContent = 'Este campo é obrigatório.';
      setTimeout(() => {
        firstEmpty.style.borderColor = '';
        firstEmpty.removeAttribute('aria-invalid');
    ...
    btn.disabled = true;
    btn.innerHTML = '<span class="btn-loading-icon">↗</span> Abrindo WhatsApp...';
